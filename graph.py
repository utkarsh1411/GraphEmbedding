...
def records_to_graph():
    ...
    for key, value in movies.items():
        movieNode = Node(id = ID, name = key, type = 'movie')
        nodedict[key] = movieNode
        nodelist.append(movieNode)
        ID = ID + 1
        for i in range (1, 6):
            movieNode = Node(id = ID, name = key+"_"+str(i), type = 'movie')
            nodedict[key+"_"+str(i)] = movieNode
            nodelist.append(movieNode)
            ID = ID + 1
	
    for dir in directors:
		dirNode = Node(id = ID, name = dir, type = 'director')
		nodedict[dir] = dirNode
		nodelist.append(dirNode)
		ID = ID + 1

    for actor in actors:
		actorNode = Node(id = ID, name = actor, type = 'actor')
		nodedict[actor] = actorNode
		nodelist.append(actorNode)
		ID = ID + 1
		
    for gen in genres:
		genNode = Node(id = ID, name = gen, type = 'genre')
		nodedict[gen] = genNode
		nodelist.append(genNode)
		ID = ID + 1		
	
    for user in ratings:
		userNode = Node(id = ID, name = user, type = 'user')
		nodedict[user] = userNode
		nodelist.append(userNode)
		ID = ID + 1
    ...
    for u in ratings.keys():
		...
		tNode = ratings[user].items()
		#uNode = 
		
		for key, value in tNode:
			rname = key + "_" + str(value)
			nodedict[user].neighbors.append(nodedict[rname])
			nodedict[rname].neighbors.append(nodedict[user])
	
    for key, value in movies.items():
        if (value.director != None):
            dirNode = nodedict[value.director]
            nodedict[key].neighbors.append(dirNode)
            dirNode.neighbors.append(nodedict[key])
		
        for actor in movies[key].actors:
            actorNode = nodedict[actor]
            nodedict[key].neighbors.append(actorNode)
            actorNode.neighbors.append(nodedict[key])
		
        for g in movies[key].genres:
            genreNode = nodedict[g]
            nodedict[key].neighbors.append(genreNode)
            genreNode.neighbors.append(nodedict[key])
		
        for r in range(1, 6):
            nodedict[key+"_"+str(r)].neighbors.append(nodedict[key])
            nodedict[key].neighbors.append(nodedict[key+"_"+str(r)])
